...
function ACWidget () {
	ACWidget.superclass.constructor.apply(this, arguments);
}
    ...
    { // prototype
        ...
        getListMarkup : function (data) {
            ...
            if(self.get("total") > self.get("data").length)
            	markup.push(self.get("moreTpl").replace("{message}",Y.Intl.get("gallery-ac-widget").morehitsavailable));
...
}, '@VERSION@' ,{requires:['overlay','gallery-ac-plugin','intl']});
