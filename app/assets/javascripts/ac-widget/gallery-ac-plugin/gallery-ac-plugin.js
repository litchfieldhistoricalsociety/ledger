...
/**
 * Because data from the RegExCache can contain values that do not exactly match the typed in value
 * we need to clean those out
 **/
function cleanQueryResponse(data,query){
	var copy = [];
	var regex = new RegExp(query,"i");
	for(var x =0;x<data.length;x++){
		if(data[x].key.match(regex) || data[x].label.match(regex))
...
function handleQueryResponse (e) {
	...
	var waitingControl = Y.one('.ui_autocomplete_loading');
	if (waitingControl)
		waitingControl.removeClass('ui_autocomplete_loading');

    // if there is a result, and it's not an empty array
    if (res && !(res && ("length" in res) && res.length === 0)) this.fire("ac:load", {
        results : cleanQueryResponse(res,this.get("queryValue")),
        query : this.get("queryValue"),
        total: e.response.meta.total
    });
}
...
function valueChangeHandler (e) {
	...
	e.currentTarget.addClass('ui_autocomplete_loading');
...
function attachHandles (self, host) {
	...
	Y.on("key", self.close, host, "down:9", self);
	//Close on enter
    Y.on('key',function(e){
    	e.preventDefault();
    	if(e.currentTarget.get("id") === host.get("id")){
    		self.close();
    		return false;
    	}
    },host,"down:13",self);
	//Close on click elsewhere in document
	Y.on("click", function(e) {
		var el = e.target;
        if (el.get("id") !== host.get("id")) {
            self.close();
        }
    },document);
}
                    ...
                    return (
                        YLang.isFunction(q) ? q :
                          function (query) {
                            ...
                            return q.replace(
                                    /(^|[^\\])((\\{2})*)\{query\}/,
                                    '$1$2'+encodeURIComponent(query)).replace(
                                    /(^|[^\\])((\\{2})*)\\(\{query\})/,
                                    '$1$2$4');
                        }
                    );
...
}, '@VERSION@' ,{requires:['node-base', 'node-pluginhost', 'plugin', 'gallery-value-change', 'event-key'], optional:['event-custom']});
